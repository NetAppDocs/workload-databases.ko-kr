...
당신은 가지고 있어야 합니다 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["_데이터베이스 호스트 생성_ 권한 부여"^] AWS 계정에서 워크로드 팩토리에 새로운 데이터베이스 호스트를 생성합니다.
...
.. *보안 그룹*: PostgreSQL에 필요한 포트 및 프로토콜을 허용하기 위해 워크로드 보안 그룹이 생성됩니다. 또한 하나 이상의 보안 그룹을 선택할 수 있습니다.
...
+
GovCloud 계정의 경우, 자격 증명에 대한 AWS SSM Parameter ARN 값을 다음 형식으로 입력하세요: `{fsx:{"username":"fsxadmin","password":"pass"}}`. AWS SSM Parameter 유형은 "Secure String"이어야 하며 이름은 다음 형식이어야 합니다 `/netapp/wlmdb/<anyname>`. SSM 파라미터 검색에 대한 자세한 내용은 link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 문서"^]에서 확인하세요.
...
... * 새 FSx for ONTAP 생성 *: 사용자 이름과 암호를 입력합니다.
+
+ GovCloud 계정의 경우 다음 형식으로 FSx for ONTAP 자격 증명에 대한 AWS SSM Parameter ARN 값을 입력합니다. `{fsx:{"username":"fsxadmin","password":"pass"}}` AWS SSM Parameter 유형은 "Secure String"이어야 하며 이름은 다음 형식이어야 합니다. `/netapp/wlmdb/<anyname>` link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 문서"^]에서 SSM 파라미터 검색에 대해 자세히 알아보십시오.
...
.. *Simple Notification Service*: 선택적으로 드롭다운 메뉴에서 PostgreSQL용 SNS 주제를 선택하여 이 구성에 대해 Simple Notification Service(SNS)를 활성화할 수 있습니다.
+
