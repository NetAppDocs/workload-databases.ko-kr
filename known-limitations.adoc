...
Terraform을 사용한 아웃바운드 연결 없이 개인 네트워크에 Microsoft SQL Server를 배포하는 것은 us-east-1(N. Virginia) 지역에서만 지원됩니다. PostgreSQL은 Amazon CloudWatch 또는 사설 네트워크에서의 배포를 지원하지 않습니다.
