...
Workload Factory 콘솔 내의 데이터베이스 인벤토리에서 구성 문제에 대한 권장 사항을 검토하고 해당 문제를 해결할 수 있습니다.

SQL Server 리소스의 경우 데이터베이스 구성에 대한 모범 사례를 검토하기 위해 일회성 평가를 수행할 수 있습니다. 일회성 평가를 수행하기 위해 리소스를 등록하거나 자격 증명을 저장할 필요는 없습니다.
...
== SQL Server 데이터베이스 구성에 대한 일회성 평가를 수행합니다

자격 증명을 저장하거나 인스턴스를 등록하지 않고 Microsoft SQL Server 인스턴스의 well-architected 상태를 확인하기 위해 일회성 평가를 수행합니다.

스크립트를 실행하고 결과를 업로드하는 횟수는 원하는 만큼 반복할 수 있습니다. 업로드할 때마다 최신 결과를 기반으로 잘 설계된 아키텍처의 상태 및 인사이트가 업데이트됩니다.
...
. *Inventory*에서 *One-time assessment* > *Learn about assessment*를 선택하세요. 일회성 평가에 대한 정보를 검토하세요.
. * 스크립트 다운로드 * 를 선택합니다.
+
...
. 모든 구성에 대한 권장 사항을 확인합니다. 권장 사항에는 모범 사례와 구성이 최적화되지 않은 경우 발생할 수 있는 문제가 포함됩니다.
...
NOTE: 구성을 수정하면 인스턴스 다운타임이나 서비스 중단이 발생할 수 있습니다. 구성을 수정하기 전에 권장 사항을 주의 깊게 검토해야 합니다.
