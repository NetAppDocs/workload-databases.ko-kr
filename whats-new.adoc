...
Workload Factory는 이제 필요할 때마다 SQL Server 데이터베이스 배포 구성을 분석하는 일회성 평가를 제공합니다. 신규 배포, 마이그레이션 후, 문제 해결 등 다양한 상황에서 활용할 수 있습니다. 배포 구성을 최적화하고 모범 사례를 준수하는 데 도움이 되는 권장 사항을 받아볼 수 있습니다. 이는 설정 프로세스에 완전히 전념하지 않고도 잘 설계된 기능을 사용할 수 있는 기회입니다. Workload Factory에 자격 증명을 저장하거나 데이터베이스 리소스를 등록하거나 외부 연결을 설정하지 않고도 데이터베이스를 평가할 수 있습니다.
