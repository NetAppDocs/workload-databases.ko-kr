...
Workload Factory는 이제 필요할 때마다 데이터베이스 배포 구성을 분석할 수 있는 일회성 평가 기능을 제공합니다. 신규 배포, 마이그레이션 후, 문제 해결 등 다양한 상황에서 활용할 수 있습니다. 평가 결과를 통해 배포 구성을 최적화하고 모범 사례를 준수하는 데 도움이 되는 권장 사항을 받아볼 수 있습니다. 이 기능을 통해 복잡한 설정 과정 없이도 잘 설계된 기능을 활용해 볼 수 있습니다. Workload Factory에 자격 증명을 저장하거나, 데이터베이스 리소스를 등록하거나, 외부 연결을 설정할 필요 없이 데이터베이스를 평가할 수 있습니다.
...
=== SQL Server 온프레미스 데이터베이스 환경 비용 분석 및 마이그레이션 계획

BlueXP 워크로드 팩토리(데이터베이스용)는 이제 온프레미스 SQL Server 데이터베이스를 Amazon FSx for NetApp ONTAP로 마이그레이션하기 위해 감지, 분석 및 계획을 지원합니다. 비용 절감 계산기를 사용하여 클라우드에서 온프레미스 SQL Server 데이터베이스 환경을 운영하는 데 드는 비용을 추산하고 온프레미스 데이터베이스 환경을 클라우드로 마이그레이션하기 위한 권장 사항을 확인할 수 있습니다.
